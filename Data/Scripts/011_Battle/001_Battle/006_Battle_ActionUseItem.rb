class Battle
  ...
  #=============================================================================
  def pbCanUseItemOnPokemon?(item, pkmn, battler, scene, showMessages = false)
    if !pkmn || pkmn.egg?
  ...
  def pbUsePokeBallInBattle(item, idxBattler, userBattler)
    ...
    pbThrowPokeBall(battler.index,item) if ItemHandlers.triggerCanUseInBattle(item, battler.pokemon, battler, @choices[userBattler.index][3], true, self, @scene, false)
