...
class Battle::Move::DoublePowerIfTargetNotActed < Battle::Move
  def pbBaseDamage(baseDmg, user, target)
    return baseDmg if target.nil?
    return baseDmg if target.index == nil
    if @battle.choices[target.index][0] == :None || ([:UseMove].include?(@battle.choices[target.index][0]) && !target.movedThisRound?) || [:SwitchOut].include?(@battle.choices[target.index][0])
      baseDmg *= 2
    end
    return baseDmg
  end
