...
#      :FORECAST,                                          # This can be stopped
  ...
  def trappedInBattle?
    ...
    return true if @battle.allOtherSideBattlers.any? { |b| b.hasActiveAbility?(:ARENATRAP) && !self.airborne? && !self.hasType?(:GHOST) && !self.hasActiveItem?(:SHEDSHELL) }
    return true if @battle.allOtherSideBattlers.any? { |b| b.hasActiveAbility?(:SHADOWTAG) && !self.hasType?(:GHOST) && !self.hasActiveItem?(:SHEDSHELL) }
    return false
  end
